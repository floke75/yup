...
- **`'sleep': identifier not found` during the audio device build** – The Windows toolchain
  ...
  Windows API.
- **Direct3D 11 initialisation fails with `bad allocation`** � We are still working against a non-headless Windows 11 workstation with an NVIDIA GeForce RTX 5090. Use the refreshed diagnostics helper (`python tools/check_d3d11_device.py`) to confirm that both hardware and WARP drivers succeed at feature level 0x0000B000; a reference run is captured in `docs/troubleshooting_ground_truth.md`. When chasing adapter issues locally, set `YUP_RIVE_FORCE_WARP=1` to skip hardware creation and `YUP_RIVE_ENABLE_D3D_DEBUG=1` to request the debug layer�both flags are now echoed in `RiveOffscreenRenderer::get_diagnostics()`. Even with these toggles the constructor can still raise `ValueError: bad allocation` before Direct3D initialises, so capture the console log plus any diagnostics and append them to the troubleshooting log for the next pass.
