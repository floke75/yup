...
- **Device diagnostics**: RiveOffscreenRenderer logs adapter names, feature levels, and HRESULT values during initialisation and can optionally mirror frames in a presentation window so GPU issues are easier to reproduce without leaving the offscreen pipeline.
...
- Attempted to load `RiveOffscreenRenderer(1280,720, enable_presentation=True)` from the freshly rebuilt wheel (imported via the build artefacts to pick up the new logging). The constructor still raises `ValueError: Failed to initialise RiveOffscreenRenderer: bad allocation`; no presentation window is created, confirming the failure happens before the new D3D11 device logging kicks in.
- Added a presentation toggle to RiveOffscreenRenderer plus a `set_presentation_enabled` binding and CLI flag (`--present-preview`) so we can mirror frames locally while keeping NDI streams headless. Logging now captures adapter descriptions, feature levels, and HRESULTs for each device creation attempt.
...
- Instrumented initialisation further: diagnostics now track DXGI adapter enumeration, feature levels, and every HRESULT via `get_diagnostics()`. Messages are mirrored to `std::cerr` so failures surface even when the logger output is not captured.
- Added `tools/check_d3d11_device.py` to verify hardware/WARP device creation without involving Rive; use it from a native Windows shell before chasing renderer-specific issues. Results should be recorded in `docs/troubleshooting_ground_truth.md`.
- Created `docs/troubleshooting_ground_truth.md` to preserve environment facts (non-headless RTX 5090 workstation), diagnostic commands, and outstanding GPU questions so future sessions avoid repeating disproven assumptions.
- Captured new diagnostics: the standalone checker reports `0x80070057 (The parameter is incorrect.)` for both hardware and WARP drivers, and a direct renderer construction attempt now surfaces `CreateTexture2D (staging) failed (0x8007000E)`, pointing at staging allocation pressure rather than missing adapters.
- Mirrored the upstream adapter-enumeration flow: the renderer now builds a DXGI factory, iterates adapters, and creates devices via `D3D_DRIVER_TYPE_UNKNOWN` before falling back to the driver-type pathway. Creation flags were trimmed to match the upstream (`0`/debug only), and load calls emit explicit logs so we can confirm `.riv` assets are actually read.
- Rebuilt the native targets after the enumeration changes; `tools/check_d3d11_device.py` still returns `0x80070057` for both hardware and WARP, and `tmp_run_renderer.py` continues to surface `ValueError: bad allocation` without new console diagnostics.

### Outstanding Issues
- Current host (equipped with an NVIDIA GeForce RTX 5090) still raises `ValueError: Failed to initialise RiveOffscreenRenderer: bad allocation`. Hardware and WARP paths refuse to create a D3D11 device, but the new diagnostics capture adapter names, feature levels, and HRESULTs for each attempt. Use those logs (and, if possible, the presentation mirror) to confirm whether adapter selection or session policies are blocking offscreen creation.
...
- After the instrumentation lands, rerun the Release build (cmake --build build --config Release) on this RTX 5090 workstation, smoke the example script + CLI (with --present-preview enabled), and record the captured HRESULT/adapter logs for the troubleshooting docs.
...
- Run `python tools/check_d3d11_device.py` from a Windows shell to capture baseline hardware vs WARP behaviour and archive the output alongside renderer diagnostics.
